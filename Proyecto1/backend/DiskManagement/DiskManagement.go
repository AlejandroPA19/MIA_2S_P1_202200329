...
func Mkdisk(size int, fit string, unit string, path string, messages *[]string) {
	...
	if size <= 0 {
		...
		*messages = append(*messages, "Error: Size debe ser mayo a  0")
		return
	}
...
func Rmdisk(path string, messages *[]string) {
    fmt.Println("======INICIO RMDISK======")
    fmt.Println("Path:", path)

    // Confirmar si el archivo existe antes de eliminarlo
    if _, err := os.Stat(path); os.IsNotExist(err) {
        fmt.Println("Error: El archivo no existe")
        *messages = append(*messages, "Error: El archivo no existe")
        return
    }
    ...
    if err != nil {
        ...
        *messages = append(*messages, "Error al eliminar el archivo: "+err.Error())
        return
    }

    fmt.Println("Disco eliminado correctamente en", path)
    *messages = append(*messages, "Disco eliminado correctamente en "+path)
}
