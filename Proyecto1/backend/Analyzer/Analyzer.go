...
import (
	...
	"github.com/AlejandroPA19/MIA_2S_P1_202200329/FileSystem"
	"github.com/AlejandroPA19/MIA_2S_P1_202200329/Structs"
	"github.com/AlejandroPA19/MIA_2S_P1_202200329/User"
...
func AnalyzeCommnad(command string, params string, messages *[]string) {
...
func fn_mkdisk(params string, messages *[]string) {
	// Definir flag
	...
	*/
	...
	if *path == "" {
		...
		*messages = append(*messages, "Error: Path is required")
		fmt.Println(messages)
		return
	}

	// LLamamos a la funcion
	DiskManagement.Mkdisk(*size, *fit, *unit, *path, messages)
}


func fn_rmdisk(params string, messages *[]string) {
    // Definir flag
    ...
    // Procesar los flags
    for _, match := range matches {
        flagName := match[1] // Capturar el nombre del flag (en este caso será "path")
    ...
    // Validación del path
    if *path == "" {
        fmt.Println("Error: Se requiere la ruta (path) para eliminar el disco")
        *messages = append(*messages, "Error: Path es requerido para eliminar el disco")
        return
    }

    // Llamamos a la función de eliminación del disco
    DiskManagement.Rmdisk(*path, messages)
}
